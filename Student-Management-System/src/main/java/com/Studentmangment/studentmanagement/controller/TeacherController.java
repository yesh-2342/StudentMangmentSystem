...
public class TeacherController {
	...
	public String showTeacherDashboard(javax.servlet.http.HttpSession session, java.security.Principal principal) {
		Teacher teacher = (Teacher) session.getAttribute("user");
		if (teacher == null && principal != null) {
			teacher = teacherService.findByTeacherName(principal.getName());
			if (teacher != null) {
				session.setAttribute("user", teacher);
			}
		}
		if (teacher != null) {
			return "redirect:/teacher/" + teacher.getId() + "/courses";
		}
	...
	public String showTeacherCourses(@PathVariable("teacherId") int teacherId, Model theModel) {
		...
		List<Course> courses = teacher != null ? teacher.getCourses() : new java.util.ArrayList<>();
		
		int totalCourses = courses != null ? courses.size() : 0;
		java.util.Set<Integer> uniqueStudentIds = new java.util.HashSet<>();
		int totalAssignments = 0;
		int upcomingAssignments = 0;
		java.util.List<java.util.Map<String, Object>> teacherAssignmentsList = new java.util.ArrayList<>();
		...
		if (courses != null) {
			for (Course course : courses) {
				...
				if (students != null) {
					...
					if (students.size() > 0) {
						StudentCourseDetails scd = studentCourseDetailsService.findByStudentAndCourseId(students.get(0).getId(), course.getId());
						if (scd != null && scd.getAssignments() != null) {
							for (Assignment assignment : scd.getAssignments()) {
								totalAssignments++;
								...
								teacherAssignmentsList.add(map);
							}
						}
					}
				}
		...
		theModel.addAttribute("totalStudents", uniqueStudentIds.size());
		theModel.addAttribute("totalAssignments", totalAssignments);
		theModel.addAttribute("upcomingAssignments", upcomingAssignments);
		theModel.addAttribute("teacherAssignmentsList", teacherAssignmentsList);
